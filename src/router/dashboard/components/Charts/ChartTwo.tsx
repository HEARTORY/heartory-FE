...
const ChartTwo: React.FC<Props> = (props) => {
  const options: ApexOptions = {
    ...
    xaxis: {
      categories: props.monthlySubscriptionByDay.map((item: any) => item[0]),
    },
    legend: {
      position: 'top',
    ...
    series: [
      {
        ...
        data: props.monthlySubscriptionByDay.map((item: any) => {
          return item[1];
        }),
      },
      {
  ...
  return (
    <div className="col-span-12 rounded-sm border border-stroke bg-white p-7.5 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-4">
      <div className="mb-4 justify-between gap-4 sm:flex">
        <div>
          <h4 className="text-xl font-semibold text-[#3C50E0] dark:text-white">
            Subscription detail
          </h4>
        </div>
        <div>
